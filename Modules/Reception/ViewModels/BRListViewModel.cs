...
public partial class BRListViewModel : BaseViewModel
{
    ...
    public ObservableCollection<BonReception> Items { get; } = [];
    [ObservableProperty] private BonReception? _selected;
    ...
    private async Task ExportPdfAsync(CancellationToken cancellationToken)
    {
        ...
        {
            await using var db = await _dbFactory.CreateDbContextAsync(cancellationToken);
            var b = await db.BonsReception.Include(x => x.Lignes).FirstAsync(x => x.Id == Selected.Id, cancellationToken);
