...
public partial class BCListViewModel : BaseViewModel
{
    ...
    public ObservableCollection<BonCommande> Items { get; } = [];
    [ObservableProperty] private BonCommande? _selected;
    ...
    private async Task ExportPdfAsync(CancellationToken cancellationToken)
    {
        ...
        {
            await using var db = await _dbFactory.CreateDbContextAsync(cancellationToken);
            var b = await db.BonsCommande.Include(x => x.Lignes).FirstAsync(x => x.Id == Selected.Id, cancellationToken);
